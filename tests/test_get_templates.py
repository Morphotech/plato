...
def populate_db(fastapi_client_local_storage: TestClient, db: Session):
    for i in range(NUMBER_OF_TEMPLATES):
        t = Template(
            id_=str(i),
            schema={"type": "object", "properties": {f"{i}": {"type": "string"}}},
            type_="text/html",
            metadata={},
            example_composition={},
            tags=[f"tag{i!s}", "example"],
        )
        db.add(t)
    db.commit()
...
class TestGetTemplates:
    ...
    def test_obtain_template_info_by_id_ok(
        self, fastapi_client_local_storage: TestClient
    ):
        tentative_template_id = 39
        assert tentative_template_id < NUMBER_OF_TEMPLATES

        response = fastapi_client_local_storage.get(
            f"{self.GET_TEMPLATES_ENDPOINT}{tentative_template_id}"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_obtain_template_info_by_id_not_found(
        self, fastapi_client_local_storage: TestClient
    ):
        tentative_template_id = 200
        assert tentative_template_id > NUMBER_OF_TEMPLATES
        response = fastapi_client_local_storage.get(
            self.GET_TEMPLATES_BY_ID_ENDPOINT.format(tentative_template_id)
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json() == {
            "detail": f"Template '{tentative_template_id}' not found"
        }
    ...
    def test_obtain_template_by_tags(self, fastapi_client_local_storage: TestClient):
        for i in range(NUMBER_OF_TEMPLATES):
            ...
            response = fastapi_client_local_storage.get(
                self.GET_TEMPLATES_ENDPOINT, params=tags
            )
            assert response.status_code == status.HTTP_200_OK
    ...
    def test_obtain_template_by_tags_empty(
        self, fastapi_client_local_storage: TestClient
    ):
        template_id = 67
    ...
    def test_obtain_template_by_more_than_one_tag(
        self, fastapi_client_local_storage: TestClient
    ):
        template_id = 32
        assert template_id < NUMBER_OF_TEMPLATES
