...
from sqlalchemy import cast as db_cast
...
from app.schemas.template_detail import MIMETypeEnum, TemplateDetailSchema
...
def template_by_id(
    template_id: str, db: Annotated[Session, Depends(get_db)]
) -> Template:
...
def templates(
    ...
    if tags:
        template_query = template_query.filter(
            Template.tags.contains(db_cast(tags, ARRAY(String)))
        )
...
def compose_file(
    ...
    db: Annotated[Session, Depends(get_db)],
    custom_accept: Annotated[str | None, Header(...)] = None,
) -> StreamingResponse:
    return _compose(
        db, jinja_env, lambda t: payload, template_id, "compose", custom_accept
    )
...
def example_compose(
    ...
    return _compose(
        db,
        jinja_env,
        lambda t: t.example_composition,
        template_id,
        "example",
        custom_accept,
    )
...
def _compose(
    ...
    template_model: Template | None = (
        db.query(Template).filter_by(id=template_id).one_or_none()
    )
